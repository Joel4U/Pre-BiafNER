...
from src.data.data_utils import build_deplabel_idx, root_dep_label, build_pos_idx, build_spanlabel_idx
...
def convert_instances_to_feature_tensors(instances: List[Instance], tokenizer: PreTrainedTokenizerFast,
                                         deplabel2idx: Dict[str, int], pos2idx: Dict[str, int], label2idx: Dict[str, int]) -> List[Dict]:
    features = []
    ...
    for idx, inst in enumerate(instances):
        ...
        features.append({"input_ids": res["input_ids"], "attention_mask": res["attention_mask"],
                         ...
                         "dephead_ids": dephead_ids, "deplabel_ids": deplabel_ids, "spanlabel_ids": spanlabel_ids})
    return features
...
def batch_variable(batch:List[Dict], config):
    ...
    if config.sb_epsilon <= 0:
        ...
        spanlabel_ids_tensor = torch.zeros((batch_size, max_seq_len, max_seq_len, len(config.label2idx)),
    ...
    for i, feature in enumerate(batch):
        ...
        if config.sb_epsilon <= 0:
            ...
            spanlabel_ids = np.zeros((max_seq_len, max_seq_len, len(config.label2idx)), dtype=np.float32)
            for span, label in feature["spanlabel_ids"].items():
                start, end = span
            ...
            spanlabel_ids[:, :, config.label2idx['O']] = 1 - np.sum(spanlabel_ids, axis=-1)
...
class NERDataset(Dataset):
    ...
    def __init__(self, file: str, enc_type: str, tokenizer: PreTrainedTokenizerFast,
                 sb_epsilon: float,  is_train: bool=True,
                 label2idx: Dict[str, int] = None,
                 deplabel2idx: Dict[str, int] = None, pos2idx: Dict[str, int] = None, is_json: bool = False):
        ...
        if is_train:
            if label2idx is not None:
                ...
                if enc_type == 'synlstm':
                    ...
                    self.deplabel2idx = None
                    self.root_dep_label_id = None
            ...
            if enc_type == 'synlstm':
                ...
                self.deplabel2idx = None
        self.insts_ids = convert_instances_to_feature_tensors(insts, tokenizer, self.deplabel2idx, self.pos2idx, label2idx)
            ...
            for line in tqdm(f.readlines()):
                ...
                if line == "" and len(words) > 1:
                    ...
                    if 'msra' in file or 'Weibo' in file or 'resume' in file:
                        insts.append(Instance(words=words, ori_words=ori_words, pos=None, depheads=None, deplabels=None, span_labels=chunks, labels=labels))
                    elif 'conll' in file:
                        insts.append(Instance(words=words, ori_words=ori_words, pos=tags, depheads=None, deplabels=None, span_labels=chunks, labels=labels))
                    else:
                        insts.append(Instance(words=words, ori_words=ori_words, pos=tags, depheads=depheads, deplabels=deplabels, span_labels=chunks, labels=labels))
                    words = ['<'+root_dep_label.lower()+'>']
                    ori_words = ['<'+root_dep_label.lower()+'>']
                    tags = [root_dep_label]
                    depheads = [0]
