...
from src.config.utils import timestep_dropout
...
class BiaffineDecoder(nn.Module):
    def __init__(self, config, size_emb_dim: int = 25):
        ...
        self.out_dim = config.affine_outputsize
        self.bias_x = True
        self.bias_y = True
        self.out_size = config.label_size
        ...
        self.affine_start = FFNEncoder(in_dim=config.context_outputsize * 2, out_dim=self.out_dim, dropout=0, activation=self._act)
        self.affine_end = FFNEncoder(in_dim=config.context_outputsize * 2, out_dim=self.out_dim, dropout=0, activation=self._act)
